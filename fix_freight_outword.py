"""
Run: python fix_freight_outword.py
Fix: Freight Outword → Direct Incomes (Tally Image 3 confirms this)
Also: POWER & FUEL and RATES AND TAXES → tag as _GROUP_TOTAL_ (sub-group totals)
"""
import sqlite3, os
DB = os.path.join(os.path.dirname(__file__), 'data', 'mis_portal.db')
conn = sqlite3.connect(DB)

print("=== FIXES ===\n")

# Fix 1: Freight Outword → Direct Incomes
cur = conn.execute("""
    UPDATE pl_data
    SET tally_group='Direct Incomes', mis_group='Direct Incomes'
    WHERE ledger_name='Freight Outword'
""")
print(f"  ✅ Freight Outword → Direct Incomes: {cur.rowcount} rows")

# Fix 2: POWER & FUEL → _GROUP_TOTAL_ (sub-group, children are individual ledgers)
cur = conn.execute("""
    UPDATE pl_data
    SET mis_group='_GROUP_TOTAL_'
    WHERE ledger_name='POWER &amp; FUEL EXPENSES'
      AND LOWER(tally_group)='indirect expenses'
""")
print(f"  ✅ POWER & FUEL → _GROUP_TOTAL_: {cur.rowcount} rows")

# Fix 3: RATES AND TAXES → _GROUP_TOTAL_
cur = conn.execute("""
    UPDATE pl_data
    SET mis_group='_GROUP_TOTAL_'
    WHERE ledger_name='RATES AND TAXES'
      AND LOWER(tally_group)='indirect expenses'
""")
print(f"  ✅ RATES AND TAXES → _GROUP_TOTAL_: {cur.rowcount} rows")

conn.commit()

print("\n=== FINAL P&L VERIFICATION ===\n")

oh = conn.execute("""
    SELECT SUM(ABS(net)) FROM pl_data
    WHERE company_id=2
      AND LOWER(tally_group) IN ('indirect expenses','salaries and bonus','salary accounts')
      AND mis_group != '_GROUP_TOTAL_'
      AND ((year=2025 AND month>=4) OR (year=2026 AND month<=3))
""").fetchone()[0] or 0

rev = conn.execute("""
    SELECT SUM(ABS(net)) FROM pl_data
    WHERE company_id=2 AND LOWER(tally_group)='sales accounts'
      AND LOWER(ledger_name)='sales accounts'
      AND ((year=2025 AND month>=4) OR (year=2026 AND month<=3))
""").fetchone()[0] or 0

# Direct Incomes — group total preferred
di_grp = conn.execute("""
    SELECT SUM(ABS(net)) FROM pl_data
    WHERE company_id=2 AND LOWER(tally_group)='direct incomes'
      AND LOWER(ledger_name)='direct incomes'
      AND ((year=2025 AND month>=4) OR (year=2026 AND month<=3))
""").fetchone()[0] or 0

di_items = conn.execute("""
    SELECT SUM(ABS(net)) FROM pl_data
    WHERE company_id=2 AND LOWER(tally_group)='direct incomes'
      AND LOWER(ledger_name) != 'direct incomes'
      AND mis_group != '_GROUP_TOTAL_'
      AND ((year=2025 AND month>=4) OR (year=2026 AND month<=3))
""").fetchone()[0] or 0

di = max(di_grp, di_items)

cos = conn.execute("""
    SELECT SUM(ABS(net)) FROM pl_data
    WHERE company_id=2 AND LOWER(tally_group)='cost of sales :'
      AND LOWER(ledger_name)='cost of sales :'
      AND ((year=2025 AND month>=4) OR (year=2026 AND month<=3))
""").fetchone()[0] or 0

ii = conn.execute("""
    SELECT SUM(ABS(net)) FROM pl_data
    WHERE company_id=2 AND LOWER(tally_group)='indirect incomes'
      AND mis_group != '_GROUP_TOTAL_'
      AND ((year=2025 AND month>=4) OR (year=2026 AND month<=3))
""").fetchone()[0] or 0

gp  = rev + di - cos
np_ = gp + ii - oh

print(f"  {'':20s} {'PORTAL':>10s}  {'TALLY':>10s}  {'DIFF':>8s}  STATUS")
print("  " + "-"*60)
for name, portal, tally in [
    ('Revenue',    rev/1e7, 68.00),
    ('Dir Inc',    di/1e7,   3.93),
    ('COGS',       cos/1e7, 52.45),
    ('GP',         gp/1e7,  19.49),
    ('Ind Inc',    ii/1e7,   0.16),
    ('Overhead',   oh/1e7,   7.20),
    ('Net Profit', np_/1e7, 12.44),
]:
    diff = portal - tally
    ok = '✅' if abs(diff) < 0.05 else ('🟡' if abs(diff) < 0.20 else '❌')
    print(f"  {name:20s} {portal:>10.2f}  {tally:>10.2f}  {diff:>+8.2f}  {ok}")

conn.close()
print("\nDone! Restart portal to see changes.")
