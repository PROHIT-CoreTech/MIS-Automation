"""Run: python fix_final2.py
Fix 1: Indirect Expenses group total capture BEFORE mis_group check
Fix 2: Verify COGS correct in updated calc
"""
import sqlite3, os
...
def calc(cid, fy, fm, ty, tm):
    ...
    for r in rows:
        ...
        # Indirect Expenses group total (regardless of mis_group tag)
        if tg == 'indirect expenses' and ln == 'indirect expenses':
            oh_grp += v; oh_found=True; continue

        # Salaries and Bonus group total (regardless of mis_group tag)
        if tg == 'salaries and bonus' and ln == 'salaries and bonus':
            oh_sal_grp += v; oh_sal_found=True; continue

        # Cost of Sales group total (regardless of mis_group tag)
        if tg == 'cost of sales :' and ln == 'cost of sales :':
            cos += v; cos_found=True; continue

        # Sales Accounts group total (regardless of mis_group tag)
        if tg == 'sales accounts' and ln == 'sales accounts':
            rev += v; continue

        # Direct Incomes group total (regardless of mis_group tag)
        if tg == 'direct incomes' and ln == 'direct incomes':
            dir_g += v; continue

        # ── NOW APPLY SKIP RULES ──────────────────────────────
        if mg == '_group_total_': continue

        # Skip remaining group total rows (ln==tg not already captured)
        if ln == tg: continue

        # ── INDIVIDUAL LEDGERS ────────────────────────────────
        if tg == 'sales accounts':       pass  # already handled above
        elif tg == 'direct incomes':
            dir_i += v  # individual items for fallback
...
print("="*70)
print("FINAL VALIDATION v2 — ALL COMPANIES")
print("="*70)
...
for label, cid, fy,fm,ty,tm, tally in tests:
    ...
    for name, pv, tv in [
        ...
        ('Dir Inc',  p['dir_inc']/1e7,  0),
        ('COGS',     p['cogs']/1e7,     0),
        ('GP',       p['gp']/1e7,       tally['gp']),
        ('Ind Inc',  p['ind_inc']/1e7,  0),
        ('Overhead', p['overhead']/1e7, tally['oh']),
        ('NP',       p['np']/1e7,       tally['np']),
    ]:
        if tv == 0: print(f"  {name:10s} {pv:>10.4f}  {'(info)':>10s}")
        else:
            diff = pv - tv
